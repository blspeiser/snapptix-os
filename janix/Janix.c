...
int main(int argc, char* argv[]) {
	...
	/* SSL_CTX_set_verify_depth(ctx, 1); */ 
		/* 1 := Require to be signed by a Trusted CA, i.e. is */
		...
		if (0 == pid) {	/* Child */
		   ...
		   /* SECURITY: Prevent buffer overruns or protocol corruptions */ 
			...
			if (COMMAND != PASSWORD)
				...
				fprintf(out,"%s Received: Password change request\n", response); 
			...
			case MANAGE:
				 ...
				 *          P: Change password
				 *          M: Mount specified file system
				 *          U: Mount specified file system
				 * 			Implementation Status: Untested
				 *--END PROTOCOL--*/ 
				if(COMMAND != NETWORK	&& 	COMMAND != BACKUP 	&& 
					COMMAND != EMAIL		&&  	COMMAND != PASSWORD  &&
					COMMAND != MOUNT		&& 	COMMAND != UNMOUNT   ){
						strcpy(response, PROTOCOLERROR);
						break;
				}
				/* otherwise */
				if (COMMAND==MOUNT) {
					if (0==strcmp(PARAMETER, "floppy") ) {
						err = mount("/dev/fd0","/mnt/floppy","vfat",
										MS_MGC_VAL | MS_RDONLY, NULL);
					} else if (0==strcmp(PARAMETER, "cdrom") ){
						err = mount("/dev/cdrom","/mnt/cdrom","iso9660",
										MS_MGC_VAL | MS_RDONLY, NULL);
					} else err = -1;
					if (err == -1) 
						strcpy(response, FAILED);
				...
				if (COMMAND==UNMOUNT){
					if (0==strcmp(PARAMETER, "floppy") )
						err = umount("/mnt/floppy");
					else if (0==strcmp(PARAMETER, "cdrom") ) 
						err = umount("/mnt/cdrom");
					else err = -1;
					if (err == -1) 
						strcpy(response, FAILED);
					else 
				...
				if (COMMAND==PASSWORD) {
					/* separate old and new passwords with a # */
					for(str = PARAMETER; *str!='#'; str++);
					*str = '\0'; 
					str++; 
					/* inv: PARAMETER := old, str := new */
				...
				if(err)
					...
					strcpy(response, SUCCEEDED);
				break;
			/* End MANAGE | NETWORK | BACKUP | MOUNT| UNMOUNT | EMAIL | PASSWORD Handler */
			...
			case ALPHA:
				 ...
				 * 											AlphaQuery not necessary; untouched
				 *--END PROTOCOL--*/ 
			...
			case NOCOMMAND:
				/* If you don't return a write, it can cause the sample client to hang */
			  	strcpy(response, EMPTYRESPONSE); 
				break;
